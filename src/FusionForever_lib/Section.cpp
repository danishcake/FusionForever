...
void Section::Tick(float _timespan, std::vector<Projectile_ptr>& _spawn_prj, std::vector<Decoration_ptr>& _spawn_dec, Matrix4f _transform, std::vector<Core_ptr>& _enemies, ICollisionManager* _collision_manager)
{
	...
	if(first_tick_)
	{
		...
		BOOST_FOREACH(ThrusterData td, trails_)
		{
			ThrusterTrail* trail = new ThrusterTrail(this, td.length_scale, td.width_scale, td.color, td.angle, td.position, td.angular_correction, td.antiparallel_factor);
