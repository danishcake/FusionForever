...
bool ForwardWingRH::initialised_ = false;
int ForwardWingRH::outline_dl_ = 0;
int ForwardWingRH::outline_verts_index_ = 0;
int ForwardWingRH::fill_dl_ = 0;
int ForwardWingRH::fill_verts_index_ = 0;

ForwardWingRH::ForwardWingRH(void)
: Section()
{
	if(!initialised_)
	{
	...
	health_ = 1500;
	max_health_ = health_;
	default_sub_section_position_ = Vector3f(15, -5, 0);
}
...
void ForwardWingRH::initialise_fill(void)
{
...
void ForwardWingRH::initialise_outline(void)
{
	boost::shared_ptr<std::vector<Vector3f>> temp_outline = boost::shared_ptr<std::vector<Vector3f>>(new std::vector<Vector3f>());

	temp_outline->push_back(Vector3f(0,0,0));			//0
	temp_outline->push_back(Vector3f(20, 0, 0));		//1
